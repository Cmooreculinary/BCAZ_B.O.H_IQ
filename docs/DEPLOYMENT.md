...
3. **Missing variables** — confirm `APP_ENV`, `APP_STORAGE`, and `ACCESS_TOKEN_MINUTES` exist with the values listed above. Services created manually (not from the Blueprint) may lack them.
